...
var RndB;
var SessionKey;
var authKey = '00000000000000000000000000000000'
var keyID = 0x00;
var fileID = 0x00;
var appID = new Buffer([0x30, 0x84, 0xf5]); // Casiers BDE



function decrypt(key, data, iv) {
    if(iv === undefined) {
        iv = new Buffer(16).fill(0);
    }
...
function encrypt(key, data, iv) {
    ...
    var decipher = crypto.createCipheriv('aes128', key, iv);
    decipher.setAutoPadding(false);
    return Buffer.concat([decipher.update(data), decipher.final()]);
}
                ...
                reader.connect({ share_mode : this.SCARD_SHARE_SHARED }, function(err, protocol) {
                    if (err) {
                        ...
                        async.waterfall([
                            ...
                            function(callback) {
                                console.log('\nStart auth');
                                reader.transmit(new Buffer([0xaa, keyID]), 40, protocol, function(err, data) {
                                    if (err) {
                                        console.log('\tError:', err);
                                    } else {
                                        console.log('\tData received', data);
